...
#filePath2 = "moduloss.txt"
dataTesting1 = np.loadtxt(filePath1, delimiter=" ")
#dataTesting2 = np.loadtxt(filePath2, delimiter=" ")
print(type(dataTesting1))
print(dataTesting1)
'''
conexion = pgdb.Connection(host="128.199.1.222",database="delati", user="modulo4", password="modulo4")
cur = conexion.cursor()
cur.execute("select id_oferta,id_ofertadetalle from oferta_detalle where ofertaperfil_id=13 and ind_activo is null limit 10;")
results = cur.fetchall()
num_rows = int(cur.rowcount)
arr_analysis = np.fromiter((tuple (row) for row in results), dtype='i4,i4', count=num_rows)
conexion.close()
results= np.array(results)
print(results)
#define params
k = 4 #numb of clusters
...
def buildSimmilarityMatrix(dataIn):
    ...
    result = np.matrix(np.full((nData,nData), 0, dtype=np.int32))
    print(nData)
